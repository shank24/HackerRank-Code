...
public class Calculator {
    ...
    public static int calculate(String expression) {
        ...
        int onsrcreenNumber = Integer.parseInt(exps[0]);

        for (int i = 1; i < exps.length; i=i+2) {
            MathOperation op = map.get(exps[i]);
            int enteredNumber = Integer.parseInt(exps[i + 1]);
            onsrcreenNumber = calculate(onsrcreenNumber, op, enteredNumber);

        }
        return onsrcreenNumber;
    }

    public static void addNewOperation(String key,MathOperation mathOperation){
